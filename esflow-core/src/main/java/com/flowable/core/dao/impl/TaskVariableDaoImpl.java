...
import com.flowable.core.common.utils.ReflectionUtils;
...
public class TaskVariableDaoImpl extends BaseDaoImpl<TaskVariable> implements ITaskVariableDao {
	...
	@Override
	public PageHelper<TaskVariable> loadTaskVariables(String processDefinitionId, int version, PageHelper<TaskVariable> page, String taskId)  {
	...
	public int getProcessOrder(TaskVariable bean)  {
		String sql = "SELECT MAX(NAME_ORDER) FROM RM.ACT_B_PROC_TASK_VAL WHERE " + (bean.getGroupName() == null ? "1=1" : "GROUP_NAME=?");
		sql += " AND TEMP_ID='" + bean.getProcessDefinitionId() + "' AND TASK_ID='" + bean.getTaskId() + "' AND VERSION=" + bean.getVersion();
	...
	public List<TaskVariable> getRefList(Map<String,Object> params){
		StringBuffer hql = new StringBuffer(" from TaskVariable where processDefinitionId=? and version=? and refVariable is not null and refVariable<>''");
		List<TaskVariable> list = this.find(hql.toString(),new Object[]{params.get("processId"),params.get("version")});
		return list;
	}
	
	@Override
	public TaskVariable getTaskVariableByName(String name,String taskId,String processDefinitionId){
		
		if(StringUtils.isBlank(name))
			return null;
		StringBuffer hql = new StringBuffer(" from TaskVariable where name=? and taskId = ? and processDefinitionId = ? ");
		List<TaskVariable> list= this.find(hql.toString(),new Object[]{name,taskId,processDefinitionId});
